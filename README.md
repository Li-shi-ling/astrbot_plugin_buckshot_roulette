# 恶魔轮盘 QQOfficial

QQOfficial 群聊多人无庄家恶魔轮盘插件。支持 2-10 人房间、公开弹数、道具、回合推进、淘汰和胜负结算。

当前版本：v1.2.6

## 使用 `@机器人 轮盘菜单` 来开始插件使用之旅！

菜单提供房间创建、游戏设置、轮盘帮助、轮盘道具查看和名称绑定按钮。首次使用时，请点击“名称绑定”，将自动生成的 `@机器人 名称绑定 [名称]` 中 `[名称]` 改为自己的名称后发送。

## 常用指令

```text
@机器人 轮盘绑定 昵称
@机器人 名称绑定 昵称
@机器人 轮盘我的名字
@机器人 轮盘改名 新昵称
@机器人 轮盘菜单
@机器人 轮盘帮助
@机器人 轮盘道具查看
@机器人 轮盘创建
@机器人 轮盘加入
@机器人 轮盘设置
@机器人 轮盘开始
@机器人 轮盘状态
@机器人 轮盘开枪 自己
@机器人 轮盘开枪 编号
@机器人 轮盘道具 啤酒
@机器人 轮盘道具 香烟
@机器人 轮盘道具 锯子
@机器人 轮盘道具 手铐 编号
@机器人 轮盘道具 放大镜
@机器人 轮盘道具 过期药
@机器人 轮盘道具 电话
@机器人 轮盘道具 转变器
@机器人 轮盘结束
```

`轮盘设置` 是群内公开默认设置，未开房时也可以修改；创建新房间时会复制当前默认值。数量按钮会生成带 `[数量]` 占位的指令，需要手动改成实际数字后发送。

手铐是目标型道具：可直接发送 `轮盘道具 手铐 编号`；点击手铐按钮时会先发送目标选择按钮。放大镜、过期药、电话仍是玩家个人持有和使用的道具，只是使用结果会在群里公开。

开启随机血量后，每局开始时只随机一次血量，并将相同的初始血量应用于所有玩家。道具“转变器”会反转当前第一发子弹的类型，同时兼容旧名称“转向器”和“换向器”。

按钮全部使用 QQ 官方 `action.type = 2` 指令按钮。文本指令始终是完整入口，即使客户端不显示按钮也能完成对局。

## 声明

纯粹vb,vb太好用了你知道吗()

## 许可

AGPL-3.0

## 开发者

- 开发者：Lishining
- 标语：恶魔轮盘不赖
- QQ 群：1083090761

欢迎 issue 和 PR，我看到后会认真处理。

[![Moe Counter](https://count.getloli.com/get/@li-shi-ling?theme=minecraft)](https://github.com/Li-shi-ling/astrbot_plugin_buckshot_roulette)
